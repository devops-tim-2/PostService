...
from models.models import Block, Post, User, Favorite
...
class TestFavorite:
    ...
    def test_favorite_happy(cls):
        cls.client.get(f'/api/favorite/{cls.post1.id}', headers={'Authorization': f'Bearer {cls.token_user2}', 'Content-Type': 'application/json'})

        favorite = Favorite.query.filter_by(post_id=cls.post1.id, user_id=cls.user2.id)

        assert favorite is not None


    def test_favorite_sad(cls):
        favorite_count_before = Favorite.query.count()
        cls.client.get(f'/api/favorite/{cls.post1.id}', headers={'Authorization': f'Bearer {cls.token_user3}', 'Content-Type': 'application/json'})
        favorite_count_after = Favorite.query.count()

        assert favorite_count_before == favorite_count_after